"""
Создание модели условий применения U для исследовательского симулятора.

Скрипт относится к этапу 5 программной реализации главы 3 диссертации.
Он создает модуль ConditionModel, который рассчитывает влияние условий
применения U на априорную оценку выполнения нормативного плана оператором O.
На данном этапе не моделируются случайные ошибки, контрольные процедуры,
фактическое кодирование E_h, декодирование D_h и восстановленное сообщение M'.
"""
...
def main() -> None:
    """Создает модуль ConditionModel, тесты и отчет этапа 5."""
    SRC_DIR.mkdir(parents=True, exist_ok=True)
    TESTS_DIR.mkdir(parents=True, exist_ok=True)
    REPORTS_DIR.mkdir(parents=True, exist_ok=True)

    write_text_file(
        SRC_DIR / "condition_model.py",
        '''
    ...
    write_text_file(
        TESTS_DIR / "test_stage5_condition_model.py",
        '''
    ...
    report = {
        ...
        "scientific_scope": (
            "Реализована модель условий применения U как априорное описание внешней "
            "обстановки: лимита времени, шума, доступности инструкции, рабочей нагрузки, "
            "прерываний и качества освещения. Модель корректирует расчетное время, внимание "
            "и устойчивость выполнения плана, но не генерирует ошибки, не реализует контроль "
            "K и не формирует фактическое сообщение M'."
        ),
        "syntax_report": syntax_report,
        "next_command": "python -m pytest",
    }

    report_path = REPORTS_DIR / "stage5_condition_model_report.json"
    report_path.write_text(
        json.dumps(report, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )

    print("ЭТАП 5. МОДЕЛЬ УСЛОВИЙ ПРИМЕНЕНИЯ U")
    print("=" * 50)
